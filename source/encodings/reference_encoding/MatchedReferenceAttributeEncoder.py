...
class MatchedReferenceAttributeEncoder(DatasetEncoder):

    dataset_mapping = {
        "RepertoireDataset": "MatchedReferenceAttributeRepertoireEncoder"
    }

    def __init__(self, reference_sequences: list, summary: SequenceMatchingSummaryType, attr_to_encode: str, same_length_sequence: bool, metadata_attrs_to_match: list, max_edit_distance: int, chunk_size: int, name: str = None):

        self.reference_sequences = reference_sequences
        self.summary = summary
        self.attr_to_encode = attr_to_encode
    ...
    @staticmethod
    def _prepare_parameters(max_edit_distance: int, summary: str, attr_to_encode: str, reference_sequences: dict, metadata_attrs_to_match: list, same_length_sequence: bool, chunk_size: int = 128, name: str = None):

        location = "MatchedReferenceAttributeEncoder"
    ...
    def build_object(dataset=None, **params):
        try:
            prepared_parameters = MatchedReferenceAttributeEncoder._prepare_parameters(**params)
            encoder = ReflectionHandler.get_class_by_name(MatchedReferenceAttributeEncoder.dataset_mapping[dataset.__class__.__name__],
                                                          "reference_encoding/")(**prepared_parameters)
        except ValueError:
            raise ValueError("{} is not defined for dataset of type {}.".format(MatchedReferenceAttributeEncoder.__name__,
    ...
    def _prepare_caching_params(self, dataset, params: EncoderParams):
        ...
        encoding_params_desc = {"same_length_sequence": self.same_length_sequence,
                                ...
                                "attr_to_encode": self.attr_to_encode,
